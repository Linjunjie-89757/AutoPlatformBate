...
### 0. 所有 Figma 视觉还原必须执行六表对齐法

页面、列表、表格、卡片、Tab、筛选器、表单、弹窗、抽屉、Popover、上传区、编辑器和状态页都不得先写外壳再凭肉眼补细节。实现前必须从 Design 节点和 Make 源码建立以下六张清单；任一清单缺失时，只能称为“结构迁移中”或“部分状态已实现”，不得称为“已对齐”。六张清单的粒度应与目标范围匹配：整页按区域拆分，复合组件按直接子元素拆分，重复项按每种变体和条件行拆分。

1. **结构表**：记录页面区域、父子层级、布局方式、固定 / 自适应尺寸、滚动容器、表头 / 行 / 列、卡片 / 分组、header / body / footer、字段顺序、按钮顺序和嵌套组件。浮层还要记录遮罩、定位、关闭入口和层级。必须先判断项目内是否已有同类 Figma 对齐实现；若 Design / Make 是自定义 `fixed` 浮层，应沿用项目的 `Teleport + fixed overlay + 自定义 panel` 模式，不得为了省事套用 `el-dialog` / `el-drawer` 后只覆盖表面样式。只有原设计本身与 Element Plus 结构等价时才可复用其默认 DOM。
2. **内容 / 字段表**：逐项记录标题、说明、字段、列名、标签、状态文案、按钮、图标、必填星号的位置、占位文案、默认值、最大长度、计数器、选项集合、字段或列宽、同行关系和 Design 中是否存在。不得因为后端已有字段就自行加入 Design 未表达的内容，也不得遗漏 Design / Make 明确存在的内容；比较自适应宽度时必须使用相同文案和数据。
3. **状态矩阵**：从 Make 的初始 state、`validate()`、错误对象、样式常量和条件渲染中提取 default、hover、focus、selected、error、disabled、loading、empty、success 等状态。每个状态必须分别记录外框、背景、文字、图标、辅助文案和光标。例如优先级不能只记录“四个按钮”，必须记录初始未选中态以及 P0 / P1 / P2 / P3 各自的选中背景、文字色和边框色；必填字段必须记录星号、字段级错误文案、错误边框、错误背景、修正输入后的恢复规则和底部汇总提示。
4. **交互表**：逐项抄录并解释 Make 的 `onClick`、`onChange`、`onFocus`、`onBlur`、`onKeyDown`、`onMouseEnter`、`onMouseLeave`、行点击、阻止冒泡、Tab 切换、展开收起、拖拽、分页、筛选、遮罩关闭、Esc、动态增删、提交校验、提交中禁用和成功反馈。不能只迁移最终静态画面；事件未核对完成时不得结论为交互已对齐。
5. **数值表**：对每个可见区域记录 Design 最终值、Make 原始值和 Vue 目标声明值，至少包含位置、宽高、padding、gap、border、radius、shadow、字号、字重、行高、颜色和透明度。颜色映射必须逐状态记录精确十六进制或 rgba，禁止把多色状态统一成主色，也禁止凭视觉印象选近似值。
6. **浏览器验收表**：在相同 viewport、DPR、字体加载和页面状态下，读取 Vue 最终 `getComputedStyle()` 与 `getBoundingClientRect()`，并与 Design / Make 数值表逐项比较。必须实际触发必填错误、每个选中态、focus、hover、disabled / loading、动态增删和关闭方式；只验证默认打开状态或只看到 CSS 中写了目标数值，不算完成。

迁移旧组件时增加“旧结构清理门禁”：

- **先列旧节点**：改造前列出旧组件中的 header、body、footer、错误提示、辅助文案、操作按钮、占位容器和旧样式选择器，标记它们在新 Design / Make 结构中的目标位置（保留、移动、合并或删除）。
- **再做父子层级核对**：模板替换后，按照 Design / Make 的真实父子关系从外到内复核一次；不能只确认节点“还在页面上”，必须确认它位于正确的父级和布局上下文中。例如 footer 汇总提示必须核对为 footer 的左侧子元素，不能仍留在 body 卡片底部。
- **最后清理残留**：删除已废弃的旧节点、旧变量和旧选择器；如果同一文案、class 或状态变量出现两份，必须说明哪一份生效并移除另一份，禁止依靠 CSS 覆盖或 `v-if` 偶然隐藏。
- **结构迁移后必须重做状态验收**：旧节点移动、合并或删除后，重新触发 default、error、selected、loading 等状态，确认错误文案、按钮组、间距和滚动区域没有回到旧位置。只看静态默认态不算完成。

本项目曾在“用例管理新增用例抽屉”中发生同类问题：新抽屉已按 Make 重建，但旧的汇总错误节点仍按 body 内卡片结构保留，导致“请检查标红字段后再保存”出现在测试内容区域而不是固定 footer 左侧。根因是只完成了视觉外壳迁移，没有执行旧节点清单和 footer 父子层级复核；后续任何重构或组件替换都必须执行上述四步。

所有视觉还原实施顺序固定为：

1. 获取准确 Design 节点、同状态截图和 Make 对应组件。
2. 在改代码前完成六张清单，先列差异再确定复用边界。
3. 保留真实业务模型、接口、权限和提交语义，只替换必要的展示结构与交互表达。
4. 先实现结构和默认态，再按状态矩阵逐项实现，不允许跳过 error / selected / loading 等条件状态。
5. 运行类型检查和 `git diff --check`，检查中文、作用域和共享组件影响。
6. 在真实浏览器逐状态截图、测量 computed style / bounding box；未能进入页面或缺少 Design 状态时必须标为“未验收”，不得用类型检查通过替代视觉验收。

强制停止条件：若当前实现只完成页面 / 面板位置、尺寸、header / body / footer、表格外框、卡片外框或分组结构，但尚未核对内容字段、条件渲染、颜色常量、选中态、错误态和事件绑定，必须停止使用“对齐”“复刻”“完成”等结论。用例管理新增用例抽屉曾因只迁移 `680px` 抽屉骨架，未先读取 Make 的 `validate()`、`fieldStyle()` 与 `PRIORITY_STYLE`，遗漏必填提示及 P0 / P1 / P2 / P3 红橙黄蓝状态；同类遗漏也可能发生在表格列和操作按钮、卡片内部标题和图标、Tab 各状态、筛选控件、条件行高度、空态 / 错误态等任何区域，后续所有视觉目标必须用上述六表门禁避免重复发生。

六表法在常见目标中的最低检查范围：

- **页面 / 仪表盘**：页面高度链路、区域顺序、栅格、标题、统计数字、卡片内部元素、响应式状态、loading / empty / error。
- **列表 / 表格**：列名、列宽、表头和行高、单元格 padding、条件字段、标签颜色、操作按钮数量 / 顺序 / 常显、整行点击、hover、分页和空态。
- **卡片 / 列表项**：容器之外还要检查图标 path、标题、说明、数值、标签、按钮，以及 selected / running / success / failed 等状态下所有直接子元素的变化。
- **Tab / 筛选 / 表单控件**：默认值、选项、箭头或可交互提示、active / focus / hover / disabled、错误文案、清除行为和切换后的状态恢复。
- **弹窗 / 抽屉 / Popover**：在通用六表之外，额外检查 overlay、z-index、视口定位、焦点、Esc、遮罩关闭、滚动锁定、footer 固定和嵌套浮层。
- **上传区 / 编辑器 / 动态表单**：拖拽、文件限制、工具按钮、动态增删、条件内容、校验、处理中、成功 / 失败、累计高度和滚动行为。
