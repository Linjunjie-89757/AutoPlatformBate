...
- [ ] 图标是否只凭名称或肉眼相似选取，是否已核对 SVG path。
- [ ] 运行中、成功、失败、空态等设计状态是否全部分别验证。
- [ ] 通用 `button { border: 0; }` 等规则是否覆盖局部按钮边框。
- [ ] 页面级 `font: inherit` 是否覆盖了按钮 / 输入框的字号、字重和行高，固定宽度按钮内 SVG 是否发生 flex 压缩。
- [ ] active / selected 状态是否同时核对了容器和内部标题、说明、图标。
- [ ] 页面根节点是否真正撑到主内容底部，是否存在未解释的空白带。
- [ ] 内部固定尺寸组件是否保持 Figma 尺寸，而不是被用来填补页面高度。
- [ ] 最终结论是否严格限定在实际验收过的范围内。
