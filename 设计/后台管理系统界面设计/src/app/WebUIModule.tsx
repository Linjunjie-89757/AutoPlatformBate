...
type UISubPage   = "cases"|"elements"|"records"|"env";
...
interface AIEl { id:string; name:string; elType:string; purpose:string; locatorType:string; locatorValue:string; confidence:number; adoptStatus:"adopted"|"ignored"|"pending"; }
...
function UICaseEditor({case_,onBack,onReRecord,onAppendRecord}:{case_:UICase;onBack:()=>void;onReRecord?:()=>void;onAppendRecord?:()=>void}) {
    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-3 flex-shrink-0 bg-white" style={{borderBottom:`1px solid ${T.border}`}}>
          <button onClick={onBack} className="flex items-center gap-1 text-[13px] transition-colors" style={{color:T.t3}} onMouseEnter={e=>e.currentTarget.style.color=T.cyan} onMouseLeave={e=>e.currentTarget.style.color=T.t3}>
          ...
          <button className="flex items-center gap-1.5 h-7 px-2.5 rounded-lg text-[12px] font-medium transition-colors" style={{color:T.t2,border:`1px solid ${T.border}`}} onMouseEnter={e=>e.currentTarget.style.backgroundColor="#F4F6FA"} onMouseLeave={e=>e.currentTarget.style.backgroundColor="transparent"}><SkipForward size={11}/>单步调试</button>
        ...
        {editorTab==="steps"&&(
          <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="rounded-xl overflow-hidden" style={{border:`1px solid ${T.border}`,backgroundColor:"#fff"}}>
              ...
              {/* Add step row */}
              <button className="w-full flex items-center justify-center gap-2 py-3 transition-colors text-[13px]"
            ...
            <div className="bg-white rounded-xl p-5" style={{ border:`1px solid ${T.border}` }}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <PBtn onClick={()=>deleteStep(selStep.id)} icon={Trash2} color={T.danger} variant="ghost">删除步骤</PBtn>
...
function UIElementLibrary({onAICapture}:{onAICapture:()=>void}) {
    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-2.5 flex-shrink-0" style={{borderBottom:`1px solid ${T.border}`,backgroundColor:"#FAFAFA"}}>
          ...
          <div className="flex-1"/>
          <PBtn icon={Plus} onClick={()=>{}} variant="ghost">手动添加</PBtn>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <ETable total={elements.length} cols={[
            ...
            {elements.map(el=>(
              <TR key={el.id}>
                <TD>
                  <p className="font-medium" style={{color:T.t1}}>{el.name}</p>
                ...
                <TD align="center"><span style={{color:el.refCount>10?T.cyan:T.t2,fontWeight:el.refCount>10?600:400}}>{el.refCount}</span></TD>
                <TD>
                ...
                <TD align="right">
                  <div className="flex items-center justify-end">
                    <IBtn icon={CheckCircle} label="验证元素" onClick={()=>{}}/>
                    <IBtn icon={Eye} label="查看引用" onClick={()=>{}}/>
...
export function WebUIModule() {
  ...
  const tabs:[UISubPage,string][]=[["cases","用例管理"],["elements","元素库"],["records","执行记录"],["env","环境配置"]];

  // Full-page recording modes (override normal layout)
  ...
  return(
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {sub==="cases"&&editCase&&<UICaseEditor case_={editCase} onBack={()=>setEditCase(null)} onReRecord={()=>setRecordPhase("config")} onAppendRecord={()=>setRecordPhase("config")}/>}
      {sub==="elements"&&<UIElementLibrary onAICapture={()=>setShowAI(true)}/>}
      {sub==="records"&&!viewRun&&<UIRunList onView={setViewRun}/>}
      {sub==="records"&&viewRun&&<UIRunDetail run={viewRun} onBack={()=>setViewRun(null)}/>}
      {sub==="env"&&<UIEnvConfig/>}

      {showAI&&<AICaptureDrawer onClose={()=>setShowAI(false)}/>}
