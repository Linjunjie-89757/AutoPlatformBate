...
// ─── Invite / Edit dialog ─────────────────────────────────────────────────────
function InviteDialog({onClose,user}:{onClose:()=>void;user?:SUser}){
  const[account,setAccount]=useState(user?.account??"");
  const[name,setName]=useState(user?.name??"");
  const[role,setRole]=useState(user?.role??"测试工程师");
  ...
  return(
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <div className="bg-white rounded-2xl w-[480px] pointer-events-auto" style={{boxShadow:"0 20px 60px rgba(0,0,0,0.16)"}}>
          <div className="flex items-center justify-between px-6 py-4" style={{borderBottom:`1px solid ${T.border}`}}>
            <div><div className="text-[15px] font-semibold" style={{color:T.t1}}>{isEdit?"编辑成员":"邀请成员"}</div><div className="text-[12px] mt-0.5" style={{color:T.t3}}>{isEdit?"修改成员信息和角色分配":"通过账号邀请新成员加入工作区"}</div></div>
            <IBtn icon={X} label="关闭" onClick={onClose}/>
          </div>
          <div className="px-6 py-5 flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-[12px] font-medium mb-1.5" style={{color:T.t2}}>账号 / 邮箱 <span style={{color:T.danger}}>*</span></label>
                <input className="h-8 w-full border rounded-lg px-3 text-[13px] outline-none" style={{borderColor:T.border,color:T.t1}} placeholder="name@company.com" value={account} onChange={e=>setAccount(e.target.value)}/></div>
              <div><label className="block text-[12px] font-medium mb-1.5" style={{color:T.t2}}>姓名</label>
                <input className="h-8 w-full border rounded-lg px-3 text-[13px] outline-none" style={{borderColor:T.border,color:T.t1}} placeholder="显示名称" value={name} onChange={e=>setName(e.target.value)}/></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-[12px] font-medium mb-1.5" style={{color:T.t2}}>分配角色 <span style={{color:T.danger}}>*</span></label>
                <select className="h-8 w-full border rounded-lg px-2.5 text-[13px] outline-none" style={{borderColor:T.border,color:T.t1}} value={role} onChange={e=>setRole(e.target.value)}>
                  {INIT_ROLES.map(r=><option key={r.id}>{r.name}</option>)}
                </select></div>
              <div><label className="block text-[12px] font-medium mb-1.5" style={{color:T.t2}}>所属工作区</label>
                <select className="h-8 w-full border rounded-lg px-2.5 text-[13px] outline-none" style={{borderColor:T.border,color:T.t1}}><option>X-MAN</option></select></div>
            </div>
          ...
          <div className="flex justify-end gap-2 px-6 py-4" style={{borderTop:`1px solid ${T.border}`}}>
            ...
            <PBtn color={SC} onClick={onClose}>{isEdit?"保存修改":"发送邀请"}</PBtn>
          </div>
        </div>
      </div>
    </>
  ...
  return(
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        <PBtn icon={UserPlus} color={SC} onClick={()=>setShowInvite(true)}>邀请成员</PBtn>
      </div>
      ...
      {(showInvite||editUser)&&<InviteDialog onClose={()=>{setShowInvite(false);setEditUser(null);}} user={editUser??undefined}/>}
      {confirmToggle&&(
