...
const workspaceId = 1;
const envId = 15;
const scenarioModuleId = 4;
const suiteName = '获客中心-回归套件';
const suiteModuleName = '获客中心';
const description = '由 Codex 生成，覆盖获客中心已确认的只读查询场景。';
const includedDescriptions = [
  '由 Codex 根据获客中心菜单首屏业务请求生成的可重复只读查询场景。',
  '由 Codex 根据获客中心低风险新增编辑删除接口生成的可重复闭环场景。',
];
...
VALUES (${workspaceId}, NULL, ${sqlString(suiteModuleName)}, 50);
`);
  moduleId = scalar(`
SELECT id
FROM tb_api_execution_suite_module
...
INSERT INTO tb_api_execution_suite (
  ...
  1, 300000, 0, 0,
  0, NULL, NULL, 'CODEX_CUSTOMER_CENTER', NULL,
...
WHERE workspace_id = ${workspaceId}
  AND module_id = ${scenarioModuleId}
...
if (!scenarios.length) {
  throw new Error('未找到可加入套件的获客中心 Codex 场景。');
}
