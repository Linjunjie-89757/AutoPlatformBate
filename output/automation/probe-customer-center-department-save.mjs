...
const departmentName = `AUTO_DEPT_${tag}`;
const departmentDesc = `自动化CRUD探测 ${tag}`;
...
await page.locator('input[placeholder="请输入部门名称"]').fill(departmentName);
await page.locator('textarea[placeholder="请输入来源描述"]').fill(departmentDesc);
await page.screenshot({ path: `${outDir}/department-before-save.png`, fullPage: true });
await page.getByRole('button', { name: /^确认$/ }).last().click();
await page.waitForLoadState('networkidle', { timeout: 20000 }).catch(() => {});
await page.waitForTimeout(1500);
await page.screenshot({ path: `${outDir}/department-after-save.png`, fullPage: true });

await fs.writeFile(`${outDir}/department-save-result.json`, JSON.stringify({ departmentName, departmentDesc, requests }, null, 2), 'utf8');
console.log(JSON.stringify({ departmentName, requestCount: requests.length }, null, 2));
await browser.close();
