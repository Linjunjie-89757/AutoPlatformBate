...
const outDir = 'output/automation/customer-center';
...
async function collectMenuCandidates(page) {
  ...
  const ignore = new Set(['首页', '工作台', '订单中心', '风控中心', '获客中心', '系统管理', '用户运营', '数据中台']);
  return candidates
    .filter((item) => item.rect.x < 360 && item.rect.y > 70)
    .map((item) => item.text.replace(/\s+/g, ' ').trim())
    .filter((text) => text.length <= 36 && !ignore.has(text))
...
await ensureLoggedIn(page, startUrl);
await page.screenshot({ path: `${outDir}/scan-customer-center-home.png`, fullPage: true });

const bodyText = await page.locator('body').innerText().catch(() => '');
const menus = await collectMenuCandidates(page);
await fs.writeFile(`${outDir}/customer-center-home.txt`, bodyText, 'utf8');
