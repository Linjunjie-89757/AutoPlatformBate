...
await page.waitForTimeout(1200);
let row = page.locator('.el-table__body-wrapper tr').filter({ hasText: advertiserName }).first();
...
await page.waitForLoadState('networkidle', { timeout: 20000 }).catch(() => {});
await page.waitForTimeout(1200);
row = page.locator('.el-table__body-wrapper tr').filter({ hasText: advertiserName }).first();
...
await fs.writeFile(`${outDir}/advertiser-disable-delete-ui-result.json`, JSON.stringify({ advertiserName, requests }, null, 2), 'utf8');
