...
type WorkspaceTab = 'interfaces' | 'releases' | 'references' | 'logs'
...
const activeTab = ref<WorkspaceTab>('interfaces')
const appSearch = ref('')
...
function changeTab(tab: WorkspaceTab) {
  activeTab.value = tab
  if (tab === 'references') emit('load-references')
}
...
function logResultTone(log: MockCallLogItem) {
