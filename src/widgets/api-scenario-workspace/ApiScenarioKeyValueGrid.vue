...
import type { ApiScenarioKeyValueGridProps } from './lib/apiScenarioStepConfigTypes'

withDefaults(defineProps<ApiScenarioKeyValueGridProps>(), {
  paramTypeOptions: () => [],
})
...
<template>
  <div v-if="variant === 'header'" class="request-section ms-like-table-surface ms-like-param-table ms-like-param-table--header">
    ...
    <div v-for="(row, index) in rows" :key="`scenario-header-${index}`" class="ms-like-table-row ms-like-param-table-grid ms-like-param-table-grid--header">
      ...
      <el-input v-model="row.value" placeholder="参数值" @input="handleScenarioKeyValueRowInput(rows, defaults)" />
  ...
  <div v-else class="request-section ms-like-table-surface ms-like-param-table ms-like-param-table--query">
    ...
    <div v-for="(row, index) in rows" :key="`scenario-query-${index}`" class="ms-like-table-row ms-like-param-table-grid ms-like-param-table-grid--query">
      <div class="ms-like-checkbox-cell"><el-checkbox v-model="row.enabled" @change="markScenarioDirty" /></div>
      ...
      <el-input v-model="row.value" placeholder="参数值 / {{variable}}" @input="handleScenarioKeyValueRowInput(rows, defaults)" />
